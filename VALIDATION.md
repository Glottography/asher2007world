...
```
  2. "Distance"
        ...
        Most common values:    0 (2902x)

  3. "Contained"

        Type of data:          Boolean
        Contains null values:  False
        Most common values:    True (2575x)
                               False (1914x)

Row count: 4489
```
So, out of 4489 speaker areas which are matched to a Glottolog language with point coordinate
- 2902 had a distance of 0 which means the point coordinate was contained either in the area or
  in the convex hull of the area and
- 2575 speaker areas contained the point coordinate properly.
...
```shell
csvjoin --left -c ID etc/traditional/glottolog_distance.csv etc/glottolog_distance_known.csv | csvgrep -c Distance -r"^0\.?" -i | csvgrep -i -c note -r".+" | csvsort -c Distance | csvcut -c ID | cldfbench geojson.geojson --glottolog-version v5.2 cldf/traditional -  > etc/traditional/glottolog_distance_outliers.geojson
```
...
```shell
csvjoin --left -c ID etc/traditional/multipolygon_spread.csv etc/multipolygon_spread_known.csv | csvgrep -c Spread -r"^([012])\.?" -i | csvgrep -i -c note -r".+" | csvsort -c Spread | csvcut -c ID | cldfbench geojson.geojson --glottolog-version v5.2 cldf/traditional -  > etc/traditional/multipolygon_spread_outliers.geojson
```
...
a corresponding [CLDF dataset](https://doi.org/10.5281/zenodo.17342179) has been published, we can investigate the
...
$ csvstat -c Glottolog_Languoid_Level cldf/traditional/languages.csv 
  9. "Glottolog_Languoid_Level"
        Most common values:    language (4500x)
                               family (229x)

Row count: 4610
$ csvstat -c Glottolog_Languoid_Level ../wurm1981pacific/cldf/languages.csv 
  9. "Glottolog_Languoid_Level"
        Most common values:    language (1921x)
                               family (98x)

Row count: 2935

$ csvjoin -c ID ../wurm1981pacific/cldf/languages.csv cldf/traditional/languages.csv | csvstat -c Glottolog_Languoid_Level
  9. "Glottolog_Languoid_Level"
        Most common values:    language (1564x)
                               family (93x)
...
```shell
cldfbench geojson.compare cldf/traditional/ ../wurm1981pacific/cldf/ --format tsv | csvformat -t > etc/traditional/wh_comparison.csv
```
...
```
  ...
  2. "Distance"
        ...
        Smallest value:        0
        Largest value:         3.594
        Most common values:    0 (1627x)
```
So for 1627 of the 1657 shared Glottocodes the speaker areas are overlapping (i.e. have distance 0) - which we can take as
positive validation of our dataset.

For 31, though, this was not the case. Again, we can write these shapes to [a GeoJSON file](etc/traditional/wh_nonintersecting.geojson) using `geojson.geojson`:
```shell
csvgrep -c Distance -r "^0\.0$" -i etc/traditional/wh_comparison.csv | csvcut -c Glottocode | csvformat -E | cldfbench geojson.geojson cldf/traditional/ --dataset2 ../wurm1981pacific/cldf - > etc/traditional/wh_nonintersecting.geojson
```
and investigate the cases on the map. Clearly, one category of such non-intersecting areas are the tiny islands
of Micronesia, where the Atlas polygons are sometimes  polygons are sometimes a bit "off":
...
290 more polygons in A&M! It turns out that this is the aggregated area for `aust1307` - the huge
...
```shell
cldfbench geojson.geojson cldf/traditional --dataset2 ../wurm1981pacific/cldf mort1237 sout2746 >  etc/traditional/wh_reldiff6.geojson 
```
...
The last metric computed by `geojson.compare` is the area ratio:
```shell
...
```
  5. "Area_Ratio"
        ...
        Type of data:          Number
        Smallest value:        0
        Largest value:         274.967

Row count: 1657
```

The area with almost 275 times the size in W&H than in A&M is [Ulithian](https://glottolog.org/resource/languoid/id/ulit1238) -
where A&M only give a polygon roughly covering [Fais Island](https://en.wikipedia.org/wiki/Fais_Island) while
W&H also add several atolls.
The area with ratio "0" is mapped to [Southern Jinghpaw](https://glottolog.org/resource/languoid/id/kach1280):
```shell
...
```shell
cldfbench geojson.geojson cldf/traditional --dataset2 ../wurm1981pacific/cldf kach1280 >  etc/traditional/wh_areasize.geojson
```
...
cent2136: ▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇ 9.60 
```
Larger distances can often be explained by displacement of populations during colonialism.
```shell
csvjoin --left -c ID etc/contemporary/glottolog_distance.csv etc/glottolog_distance_known.csv | csvgrep -c Distance -r"^(0|1)\.?" -i | csvgrep -i -c note -r".+" | csvsort -c Distance | csvcut -c ID | cldfbench geojson.geojson --glottolog-version v5.2 cldf/contemporary -  > etc/contemporary/glottolog_distance_outliers.geojson
```
...
```shell
csvjoin --left -c ID etc/contemporary/multipolygon_spread.csv etc/multipolygon_spread_known.csv | csvgrep -c Spread -r"^([012])\.?" -i | csvgrep -i -c note -r".+" | csvsort -c Spread | csvcut -c ID | cldfbench geojson.geojson --glottolog-version v5.2 cldf/contemporary -  > etc/contemporary/multipolygon_spread_outliers.geojson
```
